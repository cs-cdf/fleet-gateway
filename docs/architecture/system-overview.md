...
graph TB
    subgraph Clients["Clients"]
        PY["🐍 Python API<br/><code>fleet.call() / call()</code>"]
        HTTP["🌐 HTTP Client<br/><code>curl / OpenAI SDK</code>"]
        MCP["🤖 Claude Code<br/><code>llm_call / llm_analyze_files</code>"]
    end
    ...
    subgraph GW["fleet-gateway core"]
        ...
        FILES["📎 Files<br/>base64 images · text · PDF<br/>path traversal guard"]
    ...
    style Web      fill:#16213e,stroke:#4ecca3,color:#fff
```
...
| Router | `router.py` | Capability resolution, fallback chain, model index, thread-safe caching |
